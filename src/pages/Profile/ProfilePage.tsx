...
function ProfilePage() {
  ...
  const image = 'https://images.unsplash.com/photo-1560785477-d43d2b34e0df?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
  ...
  return (
    <div className="grid gap-4 p-4 max-w-7xl mx-auto">
      ...
      <div className="border shadow-sm">
        <div className="h-32 bg-cover relative bg-slate-600 bg-blend-overlay bg-center mb-8"
          style={{ backgroundImage: `url(${image})` }}
        > <img src="https://avatar.iran.liara.run/public" className="border-4 border-white w-20 h-20 rounded-full absolute -bottom-10 left-5" alt="" />

        </div>

        <div className="p-4">
          <h1 className="text-blue-500 hover:underline cursor-pointer">@{profile.nickname}</h1>
          <div>
              ...
              <tbody>
                {userTransactions.map((transaction) => (
                  <tr key={transaction.TransID} className="hover:bg-gray-100 ">
                    ...
                    <td className="px-4 py-2 border-b whitespace-nowrap">{transaction.FirstName}</td>
                    <td className="px-4 py-2 border-b whitespace-nowrap">{transaction.MSISDN}</td>
                    <td className="px-4 py-2 border-b whitespace-nowrap">{transaction.TimeStamp}</td>
                    <td className="px-4 py-2 border-b whitespace-nowrap">{transaction.TransAmount}</td>
                    <td className="px-4 py-2 border-b whitespace-nowrap">{transaction.TransID}</td>
                    {/* <td className="px-4 py-2 border-b whitespace-nowrap">{transaction.TransactionType}</td> */}
                    <td className="px-4 py-2 border-b whitespace-nowrap">{transaction.used}</td>
                  </tr>
                ))}
              </tbody>
            </table>
