...
export default function useTisiniCookies() {
    const [cookies, setCookieToken] = React.useState<TokenType | null>(Cookie.getCookieToken('ck_63hsG-sscWPkl'))
    const isCookie = React.useMemo(() => {
        if (typeof cookies === 'object') {
            const cookieAvailable = Object.keys(cookies!).length > 0 ? true : false
            if (cookieAvailable) {
                ...
                if (accessToken && refreshToken) {
                    ...
                    if (currentTime > accessTokenExpTime! || currentTime > refreshTokenExpTime!) {
                        return false
                    }
                    return true
                }
            ...
            return Object.keys(cookies!).length > 0 ? true : false
        }
        // const isCookieObj
